        ...
        const Mat &inputs, const Mat &outputs, Mat &params)
{
    ...
    for (i = 0; i < MAX_ITER; i++)
    {
        ...
        for (int j = 0; j < m; j++)
        {
            ...
            r.at<double>(j, 0) = Func(input, params) - outputs.at<double>(j, 0); //diff between previous estimate and observation population
        ...
        Mat hlm = (Jf.t() * Jf + u * I).inv() * (-Jf.t() * r);
        cv::Mat tmp_jf = (Jf.t() * Jf + u * I);
        // std::cout << "the jf.t*jf is " << tmp_jf.at<double>(0, 0) << "," << tmp_jf.at<double>(1, 1) << std::endl;
        std::cout << "the rows of hlm is " << hlm.rows << "," << hlm.cols << std::endl;
        cout << "the raw cost is " << mse << endl;
        cout << "the hlm is " << hlm.at<double>(0, 0) << "," << hlm.at<double>(1, 0) << endl;
        // return ;
        params_tmp += hlm;
        for (int j = 0; j < m; j++)
        {
